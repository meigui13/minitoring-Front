<template>
    ...
    <div>
      <iframe src = "http://localhost:7070/care/getFalldetectionStream/" id="mobsf" scrolling="no" frameborder="0"
...
export default {
  ...
  mounted(){
    ...
    // 该方法用于刷新表格
    fetchNewTable() {
      minitor.getFallList().then(res => {
        console.log("刷新表格")
        if (res.code == 200 ) {
          this.tableData = res.fall_list
        }
      })
    },
