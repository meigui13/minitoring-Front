<template>
    ...
    <div>
      <iframe src = "http://localhost:7070/care/getVolunteeractStream/" id="mobsf" scrolling="no" frameborder="0"
...
export default {
  ...
  mounted(){
    ...
    // 该方法用于刷新表格
    fetchNewTable() {
      minitor.getInteractList().then(res => {
        console.log("刷新表格")
        if (res.code == 200 ) {
          this.tableData = res.interact_list
        }
      })
    },
