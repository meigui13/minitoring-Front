<template>
    ...
    <div>
      <iframe src = "http://localhost:7070/care/getFenceinStream/" id="mobsf" scrolling="no" frameborder="0"
...
export default {
  ...
  mounted(){
    ...
    // 该方法用于刷新表格
    fetchNewTable() {
      minitor.getIntrusionList().then(res => {
        console.log("刷新表格")
        if (res.code == 200 ) {
          this.tableData = res.intrusion_list
        }
      })
    },
