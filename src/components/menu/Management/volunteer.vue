<template>
  <div>
    ...
    <el-card class="input-card">
      ...
      <el-form :inline="true" :model="checkForm" ref="checkForm" :rules="rules" label-width="120px" class="input-form">
        <el-form-item prop="id" label-position="left" class="input-el-form-item">
          <span slot="label"  style="color: #403b3b">义工id:</span>
    ...
    <el-card style="margin-top: 10px;">
      <el-button type="primary" icon="el-icon-plus" size="small" style="float:left;padding: 6px;margin-bottom: 10px;" @click="addNew">增加义工</el-button>
      <!-- 表相关 -->
      ...
      >
        ...
        <el-table-column fixed prop="ID" label="义工id" ></el-table-column>
        <el-table-column label="义工基本信息">
          <template slot-scope="scope">
            <el-popover trigger="hover" placement="top">
              <p>姓名: {{ scope.row.volunteername }}</p>
              <p>性别: {{ scope.row.sex }}</p>
              <p>电话: {{ scope.row.phone }}</p>
              <p>身份证号: {{ scope.row.idcard }}</p>
              <p>出生日期: {{ scope.row.birthday }}</p>
              <div slot="reference" class="name-wrapper">
                <el-tag type="info" effect="plain" size="medium">{{ scope.row.volunteername }}</el-tag>
              </div>
            </el-popover>
          </template>
        </el-table-column>
        <el-table-column prop="checkindate" label="访问日期"></el-table-column>
        <el-table-column prop="checkoutdate" label="离开日期"></el-table-column>
